...
class RemoteOKSource(JobSource):
    ...
    async def search(self, query: str | None) -> list[RawPosting]:
        raw_postings = await self._fetch() # RemoteOK API takes no query parameter
        ...
        postings = await self._parse(raw_postings)
        return postings
    ...
    async def _parse(self, payload: list[dict[str, Any]]) -> list[RawPosting]:
        ...
        for item in payload:
            if not isinstance(item, dict) or "id" not in item:
                continue
            try:
                postings.append(await self._parse_one(item))
            except Exception:
    ...
    @staticmethod
    async def _parse_one(item: dict[str, Any]) -> RawPosting:
        tags = item.get("tags")
        return RawPosting(
            source="remoteok",
